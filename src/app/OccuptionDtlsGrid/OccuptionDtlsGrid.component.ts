    ...
    async gridDataAPI(params, gridReqMap: Map<string, any>, event) {
        ...
        this.services.http.fetchApi('/OccupationDetails', 'GET', inputMap).subscribe(
            async (httpResponse: HttpResponse<any>) => {
                ...
                var loopDataVar4 = [];
                var loopVar4 = res['OccupationDetails'];
                if (loopVar4) {
                    for (var i = 0; i < loopVar4.length; i++) {
                        var tempObj = {};
                        tempObj['OCCUPATION_ID'] = loopVar4[i].OccupationSeq;
                        tempObj['OD_OCCUPATION'] = loopVar4[i].Occupation;
                        tempObj['OD_INDUSTRY'] = loopVar4[i].Industry;
                        tempObj['OD_COMPANY_NAME'] = loopVar4[i].CompanyName;
