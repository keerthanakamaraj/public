    ...
    isDuplicateEntrey() {
        ...
        if (OldRecords != undefined) {
            for (let eachRecord of OldRecords) {
                if (eachRecord.Id != this.HidVisitReportSeqId.getFieldValue() && eachRecord.PlaceofVisit_ID == this.VRF_PlaceOfVisit.getFieldValue() && eachRecord.NameOfPerson.replace(/\s/g, "").toUpperCase() == this.VRF_NameofPersonMet.getFieldValue().replace(/\s/g, "").toUpperCase() && eachRecord.DateOfVisit == this.VRF_DateOfVisit.getFieldValue()) {
