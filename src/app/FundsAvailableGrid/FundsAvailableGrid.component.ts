...
@ViewChildren('FundsAvailable')FundsAvailable : QueryList<TextBoxComponent>;
@ViewChildren('Amount')Amount : QueryList<AmountComponent>;
@ViewChildren('LocalCurEqui')LocalCurEqui : QueryList<AmountComponent>;
...
this.LocalCurEqui.toArray()[rowNo].setReadOnly(true);
}
...
row['LocalCurEqui_desc'] = this.LocalCurEqui.toArray()[i].getFieldInfo();
addInfo.push(row);
}
